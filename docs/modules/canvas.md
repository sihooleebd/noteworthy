...
#### `draw-arc`
Draws an arc defined by a center and two points.
```typst
#draw-arc(
  (center: (x:0, y:0), p1: (x:2, y:0), p2: (x:0, y:2)),
  theme: active-theme
)
```
The arc is drawn from `p1` to `p2`. The radius is derived from the distance between center and `p1`.
