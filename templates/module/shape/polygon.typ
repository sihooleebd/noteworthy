...
#let polygon(..points, label: none, fill: none, style: auto) = {
  let pts = points
    .pos()
    .map(p => {
...
#let triangle(p1, p2, p3, label: none, fill: none, style: auto) = {
  polygon(p1, p2, p3, label: label, fill: fill, style: style)
}
...
/// - fill: Optional fill color
#let rectangle(corner, width, height, label: none, fill: none, style: auto) = {
...
/// Create a square
#let square(center, side, label: none, fill: none, style: auto) = {
  let half = side / 2
  let c = if is-point(center) { center } else { point(center.at(0), center.at(1)) }
  polygon(
